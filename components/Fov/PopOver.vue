      ...
      <KeepAlive>
        <div class="backdrop-blur-lg" v-if="!detailsSent">
          ...
          <form class="flex flex-col gap-2 mt-4" @submit.prevent="sendDetails">
            ...
            <button type="submit" class="bg-Blue text-white p-2 rounded-lg hover:bg-blue-400 mt-2 relative min-h-10"><span>
              <SVG name="loading" v-if="loading" class="w-8 h-8 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2"/>
